...
// SUPABASE CONFIG
// ============================================================================
const SUPABASE_URL = "https://duqsnpfyybmraemqkthu.supabase.co";
const SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImR1cXNucGZ5eWJtcmFlbXFrdGh1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzM2MzQ4OTgsImV4cCI6MjA4OTIxMDg5OH0.MyEXhJThW9V8pH9dDT2ww2PK5I8B38XyhDE3K1JP0Hw";
...
function getDeviceId() {
  const id = localStorage.getItem("spirit_device_id");
  if (!id) {
...
function loadData(key, fallback) {
  try { 
    const r = localStorage.getItem("spirit_" + key); 
...
function ProgressScreen({ profile, reflections }) {
  ...
  const wc = {}; 
  reflections.forEach(r => (r.weaknesses||[]).forEach(w => { wc[w] = (wc[w]||0)+1; }));
  const sw = Object.entries(wc).sort((a,b) => b[1]-a[1]);
  const f5 = ps.slice(0, Math.min(5, ps.length)); 
  const l5 = ps.slice(-Math.min(5, ps.length));
...
export default function SpiritCoach() {
  ...
  const gen = function() { 
    const p = generateAdaptivePlan(profile, reflections, planHistory); 
    setCurrentPlan(p); 
    const h = [...planHistory, p]; 
    setPlanHistory(h); 
    saveData("plans", h); 
    syncPlanToSupabase(p); 
  };
  
  const reflect = function(r) { 
    const u = [...reflections, r]; 
    setReflections(u); 
    saveData("reflections", u); 
    syncReflectionToSupabase(r); 
    setCurrentPlan(null); 
  };
    ...
    <div style={S.app}>
      <div style={S.header}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <button style={{ background: "none", border: "none", color: T.textDim, fontSize: 16, cursor: "pointer", padding: 4 }} onClick={reset} title="Reset">{"\u2699\ufe0f"}</button>
        </div>
      </div>
