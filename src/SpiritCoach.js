...
// SUPABASE CONFIG
// ============================================================================
var SUPABASE_URL = “https://duqsnpfyybmraemqkthu.supabase.co”;
var SUPABASE_KEY = “eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImR1cXNucGZ5eWJtcmFlbXFrdGh1Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzM2MzQ4OTgsImV4cCI6MjA4OTIxMDg5OH0.MyEXhJThW9V8pH9dDT2ww2PK5I8B38XyhDE3K1JP0Hw”;
...
}, options || {})).then(function(r) { return r.json(); }).catch(function(e) { console.error(“Supabase error:”, e); return null; });
}

// Get or create a device ID so we can link data across sessions
function getDeviceId() {
var id = localStorage.getItem(“spirit_device_id”);
if (!id) {
id = “dev_” + Date.now() + “_” + Math.random().toString(36).substr(2, 9);
localStorage.setItem(“spirit_device_id”, id);
}
return id;
}

// ============================================================================
// STORAGE - localStorage (instant) + Supabase (sync in background)
// ============================================================================
function loadData(key, fallback) {
try { var r = localStorage.getItem(“spirit_” + key); return r ? JSON.parse(r) : fallback; } catch(e) { return fallback; }
}
function saveData(key, value) {
try { localStorage.setItem(“spirit_” + key, JSON.stringify(value)); } catch (e) { console.error(e); }
}

// Supabase sync functions (fire and forget - don’t block UI)
function syncProfileToSupabase(profile) {
...
sbFetch(“profiles?id=eq.” + deviceId, { method: “GET” }).then(function(data) {
if (data && data.length > 0) {
sbFetch(“profiles?id=eq.” + deviceId, { method: “PATCH”, body: JSON.stringify(row) });
} else {
sbFetch(“profiles”, { method: “POST”, body: JSON.stringify(row) });
}
});
}
...
sbFetch(“reflections”, { method: “POST”, body: JSON.stringify(row) });
}

function syncPlanToSupabase(plan) {
var deviceId = getDeviceId();
var row = {
id: plan.id,
profile_id: deviceId,
...
adaptation_note: plan.adaptationNote || “”,
drills: plan.drills || [],
insight: plan.insight || null,
based_on_reflections: plan.basedOnReflections || 0,
persistent_weaknesses: plan.persistentWeaknesses || []
};
sbFetch(“plans”, { method: “POST”, body: JSON.stringify(row) });
}
...
useEffect(() => {
var p = loadData(“profile”, null);
var r = loadData(“reflections”, []);
var h = loadData(“plans”, []);
if (p) { setProfile(p); }
setReflections(r);
setPlanHistory(h);
if (h.length > 0) {
...
var setup = function(p) { setProfile(p); saveData(“profile”, p); syncProfileToSupabase(p); setTab(“plan”); };
var gen = function() { var p = generateAdaptivePlan(profile, reflections, planHistory); setCurrentPlan(p); var h = [].concat(planHistory, [p]); setPlanHistory(h); saveData(“plans”, h); syncPlanToSupabase(p); };
var reflect = function(r) { var u = [].concat(reflections, [r]); setReflections(u); saveData(“reflections”, u); syncReflectionToSupabase(r); setCurrentPlan(null); };
var reset = function() { localStorage.removeItem(“spirit_profile”); localStorage.removeItem(“spirit_reflections”); localStorage.removeItem(“spirit_plans”); setProfile(null); setReflections([]); setPlanHistory([]); setCurrentPlan(null); setTab(“plan”); };
